...
cron.schedule('55 4 * * 1-6', async () => {
  console.log('Starting evening auction (4:55 AM WAT)');
  const startTime = new Date();
  const endTime = new Date(startTime.getTime() + 60 * 60 * 1000); // 60 minutes
  await createAuctionSession(startTime, endTime);
});
...
cron.schedule('55 5 * * *', endAuctionSession); // End evening/Sunday auction at 7:30 PM
